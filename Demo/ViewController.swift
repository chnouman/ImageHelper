...
class ViewController: UICollectionViewController {
    ...
    override func viewDidLoad() {
        ...
        if let image = UIImage(named: "beach")?.applyTintEffect(UIColor.redColor()) {
            effects.append(CellItem(text: "Tint Effect", image: image))
        }
        if let image = UIImage(named: "beach")?.applyBlur(10, tintColor: UIColor(white: 1.0, alpha: 0.3), saturationDeltaFactor: 1.8) {
